...
def train_coreset_model(args, model, train_loader, test_loader):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer=optimizer, T_max=args.epochs * len(train_loader)
    )
    ...
    for epoch in range(args.epochs):
        ...
        train_acc, train_loss = train(
            args, model, train_loader, optimizer, scheduler, criterion, epoch, log
        )
...
def train(args, model, train_loader, optimizer, scheduler, criterion, epoch, log):
    ...
    print_log(
        f"  Train Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Error@1 "
        f"{(100-top1.avg):.3f}",
        log,
    )
...
def validate(args, model, test_loader, criterion, log):
    ...
    print_log(
        f"  Test  Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Error@1 "
        f"{(100-top1.avg):.3f}",
        log,
    )
